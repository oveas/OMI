...
using \text{ALL{\_}INPUTS}, the screen is refreshed twice after the last item.
...
\paragraph{SESSION{\_}LOG{\_}LOCATION}\label{para:sessionlogfile}
...
Note the path value should ens with a colon for non-concealed logicals!
...
\paragraph{CENTER{\_}SELECT{\_}LISTS}\index{select lists!subwindow position}
...
By default, a logfile is created for each session is \textsl{OMI{\$}MENU{\_}DIRECTORY:}.

If the location of the logfile shouls be changed for all or some users, it can bve changed in the configuration file for those users (see section \ref{para:sessionlogfile}).
