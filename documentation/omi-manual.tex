...
messages. After this, OMI can crash at random moments without wa\newenvironment{absolutelynopagebreak}
...
\item When a \textsl{STRING} input is entered that contains an exclamation mark, e.q. ``\texttt{Hello! World!}'', De displayed string will differ (``\texttt{Hello! World!!75* \ensuremath{\backslash}}'' in this example) and the error \texttt{{\%}SYSTEM-F-BADPARAM, bad parameter value} will be given.
This has no influence on the actual value given to the variable name, and after a \textsl{REFRESH} command the value will look ok again.
\end{itemize}
...
\noindent\textbf{{\copyright} 1997-2026\\
...
%list by sending an email to \textsf{omi-request@oveas.com} with the word 
%\textsf{subscribe} on the subject line. \\
...
The PDF version was created with \texttt{pdflatex}.
...
DELETE {\$}1{\$}DKA200:[TEMP]OMI-V2{\_}1.ZIP;1 ? [N]: \newline
...
\begin{stayonpage}
\hspace{-8mm}\textbf{NOTE:} The file \textsl{OMI{\$}SETUP.COM} defines the foreign command \textsl{OMI} to start OMI. This command calls \textsl{OMI{\$}WRAPPER.COM} which in call the main program \textsl{OMI{\$}MENU.COM}, passing all parameters and qualifiers.\index{wrapper}\index{OMI{\$}WRAPPER}\\
To call \textsl{OMI{\$}MENU.COM} directly, modify \textsl{OMI{\$}SETUP.COM} and change the line\\
\indent\texttt{{\$}\hspace{8mm}omi :== @PROJECTS:[OMI]OMI{\$}WRAPPER.COM}\\
to\\
\indent\texttt{{\$}\hspace{8mm}omi :== @PROJECTS:[OMI]OMI{\$}MENU.COM}
\end{stayonpage}
...
\subsection{/DELAY}
\index{sessionlog!replay!delay}
\label{subsubsec:delay}
\indent\textbf{/DELAY=\textit{value}}\\*[3mm]
In replay mode (see section \ref{subsubsec:replay}), a delay can be introduces between all steps using this qualifier.\\
The value must be 0 or greater, where 0 means the user is requested to press \lt Return \gt{} after every step, all other values are interpreted as the delay in seconds.

\vs

To replay without delays, omit this qualifier.

\vs

The \textsf{/DELAY} qualifier can only be used together with \textsf{/REPLAY}.
...
\indent\textbf{/IGNORE=\textit{(keyword[,...])}}\\*[3mm]
...
\index{\%OMI-W-DUPL}
\indent\textbf{/IGNORE=DUPLICATES}\\*[3mm]
...
The \textsf{/IGNORE=DUPLICATES} qualifier overwrites all existing symbols
without a warning.
    
\subsubsection{DCLERRORS}
\index{ignore!DCL errors}
\index{qualifiers!ignore!dclerrors}
\indent\textbf{/IGNORE=DCLERRORS}\\*[3mm]
...
The \textsf{/IGNORE=DCLERRORS} qualifier ignores errors generated in such circumstances.
...
\indent\textbf{/IGNORE=DCLFATALS}\\*[3mm]
This keyword has the same effect as the DCLERRORS keyword, but for
fatal exit codes (severity 4).

Both keywords can be given together as \textsf{/IGNORE=(DCLERRORS,DCLFATALS)}.
...
\textsf{/JUMPS} can not be used together with \textsf{/REPLAY}. 
...
\subsection{/REPLAY}
\index{sessionlog!replay}
\label{subsubsec:replay}
\indent\textbf{/REPLAY=\textit{session-logfile}}\\*[3mm]
With this qualifier, a session logfile that has been recorded earlier (see section \ref{subsubsec:logsessions}) will be read and replayed.\\
The \textit{session-logfile} should be the full name, including path, of the file.\\
\textsf{/REPLAY} can not be used together with \textsf{/JUMPS} or \textsf{/SUBMENU}. 

\vs

For more information, refer to section \ref{subsubsec:autoreplay}.

\subsection{/SESSION{\_}LOG}
\index{log sessions!disable}
...
\textsf{/SUBMENU} can not be used together with \textsf{/REPLAY}. 
...
To go back one level in the menu structure, you can use the \textsf{BACK}\index{OMI 
commands!BACK} command, the \lt Ctrl/Z\gt{} keystroke, or type a \textsf{0} (zero), 
...
The \textsf{MAIN}\index{OMI commands!MAIN} command jumps immediately to the top-level 
menu from anywhere in the structure. The commands \textsf{EXIT}\index{OMI 
commands!EXIT} or \textsf{QUIT}\index{OMI commands!QUIT} end the OMI session.

\vs

The command \textsf{JUMP}\index{OMI commands!JUMP}, followed by a menu name, jumps to 
...
\subsubsection{Jumps}\index{auto mode!jumps}\label{subsubsec:autojumps}
...
Since version 2.8, the menu name can be omitted when using qualifiers: jumps will
...
\index{auto mode!replay}
\index{sessionlog!replay}
\index{replay sessionlog}
\label{subsubsec:autoreplay}

When sessions have been logged (see section \ref{subsec:logsession}), a session logfile can be used for replay using the \textsf{/REPLAY=\textit{logfile}} option when starting OMI (see alsio section \ref{subsubsec:replay}). In this mode, all previously given commands and inputs are read from the session logfile and replayed in a new session.

All screen updates will be displayed. This can be supressed using the \textsf{/BATCH} qualifier (see section \ref{subsubsec:mylabel3}).

\vs

By default, all actions actions will be performed again consecutively without delay. To replay interactive, e.g. for analysis purposes, the \textsf{/DELAY} qualifier can be used (see section \ref{subsubsec:delay}).\\
This qualifier expects an integer greater than 0 which will be the number of seconds between all actions. If a delay of 0 is given, OMI will wait for the user to press \lt Return\gt{} after every action. 

\vs

Note that session logfile by default are deleted after a given amount of time (see section \ref{subsec:cleansessionlog}). In order to use session logfiles for a longer period, the files should be saved at a different location of with a name that doesn't match the file pattern.

\vs

\hspace{-8mm}\textbf{NOTE:} Sessions replayed from previously recorded session will not be recorded again, no new session logfile will be written!
...
If a key was set using the \textsf{SET KEY}\index{OMI commands!SET KEY} command 
...
\textsf{SILENT{\_}DCL}\index{OMI commands!SILENT{\_}DCL}), the output, if any, 
...
\subsubsection{DEVICE{\_}TYPE}\index{terminal!device type}

Optionally set the terminal to a specific device type. Specify a value that is supported by the \textsf{SET TERMINAL /DEVICE{\_}TYPE} command.

E.g., for PuTTY clients\footnote{ \url{https://www.putty.org}}\index{PuTTY}, the device type \textsl{vt400} works best. With other device types, resizing the screen height might not work properly.\\
Remove or outcomment this item to use the process value.
...
Specify `\textsl{0}' to leave the current screen width unchanged.

\subsubsection{HEIGHT}

Define the number of lines the menu should use.\\
Specify `\textsl{0}' to leave the current screen height unchanged.

\subsubsection{EXIT{\_}WIDTH}

When leaving the menu, \textsl{EXIT{\_}WIDTH} is the screenwidth that is set on exit.\\
The value `\textsl{0}' can be used to restore to the original value before OMI started.
...
\subsubsection{REPLAY{\_}MODE}\index{sessionlog!replay}\index{replay sessionlog}

When replaying a session logfile using delays (see section \ref{subsubsec:autoreplay}), prompts will be displayed as in a normal session. This setting is used as a prefix for all prompts to distinguish replay sessions from a normal session.

The default value is ``\textsl{(replay mode) }''.
...
\subsection{[RESTRICTED{\_}ACCOUNTS]}
\index{security!restriced accounts}\index{restriced accounts}
\index{security!captive accounts}\index{captive accounts}\label{subsubsec:restracc}

In this section, certain rights can be given to or taken from useers that have been created with the \textsl{Restricted} flag and the defaults for user that have been created with the \textsl{Captive} flag.\\
The settings for Captive accounts cannot be changed.

\subsubsection{DCL{\_}ALLOWED}

When set to \textsl{TRUE}, the user is allowed to use the \textsl{DCL},  \textsl{SILENT{\_}DCL} or \textsl{EDIT} command (see sections \ref{subsec:mylabel12}, \ref{subsec:silent} and \ref{subsec:mylabel13}) at the \textsl{OMI} prompt. Default is \textsl{TRUE}.\\
Captive accounts are never allowed to execute these commands.

If a \textsl{COMMAND} item inside a menu file (see section \ref{subsubsec:mylabel35}) contains a \textsl{(SILENT{\_})DCL} command, that will always be executed for both Captive and Restricted users, regardless the setting.

\subsubsection{SPAWN{\_}ALLOWED}

When set to \textsl{TRUE}, the user is allowed to use the \textsl{SPAWN} command (see section \ref{subsec:spawn}) at the \textsl{OMI} prompt. Default is \textsl{FALSE}. \\
Captive accounts are never allowed to execute the \textsl{SPAWN} command.

If a \textsl{COMMAND} item inside a menu file (see section \ref{subsubsec:mylabel35}) contains a \textsl{SPAWN} command, that will always be executed for Restricted users, regardless the setting. For Captive accounts, that will be blocked by VMS.

\subsubsection{FORCE{\_}LOGOUT}

When set to \textsl{FALSE}, the user will get the normal VMS prompt when exiting the menu and is allowed to continue working in DCL.\\
Default is \textsl{TRUE}, which logs out the user when exiting the menu.

Captive accounts are logged out by the OS on exit.
...
The file pattern of the files that are cleaned is \textsl{\textit{SESSIONLOG{\_}ID}{\_}\textit{yyyymmddHHMMSSCC}.LOG}.
...
configuration file is set to \textsl{TRUE}, and with the \textsf{SHOW NAME}\index{OMI 
commands!SHOW NAME} command on the OMI prompt.

\vs

The name is used for the \textsf{JUMP}\index{OMI commands!JUMP} command (see section \ref{subsec:mylabel16}),
and in auto mode with \textsf{/JUMPS} (see section \ref{subsubsec:autojumps}).
...
the menu- name is used as a parameter! (See also section \ref{subsubsec:autojumps})
...
the example, or with the \textsf{SHOW COUNTER}\index{OMI commands!SHOW COUNTER} 
command.
...
updated until the screen is refreshed with the \textsf{REFRESH}\index{OMI 
commands!REFRESH} command, except for \textsf{INCREASE}\index{OMI commands!INCREASE} 
command, with which the \textsf{REFRESH} parameter can optionally be specified. \\
The \textsf{INCREASE}\index{OMI commands!INCREASE} command increases the current 
...
The command \textsf{SET COUNTER}\index{OMI commands!SET COUNTER}\index{menu!counter}, 
...
password can only be set or changed with the \textsf{SET PASSWORD}\index{OMI 
...
the full path and name of a DCL procedure.\\
...
\textsf{/DEBUG}\index{debug mode}\index{OMI mode!debug}.
...
to OMI modules. Up til OMI V2.5, the keyword \textsf{INPUT} had to be followed by an integer, starting with `1', and without gaps in the counting, so if a menu had three input 
...
OMI exits, and so will be available for a future session.\\
...
When this input is asked using the \textsf{ALL}\index{OMI commands!ALL} command, or 
...
When the input item is a Free input type (see section \ref{subsubsec:mylabel37}), the default will be the exact
...
If a menu contains an on-the-fly menu (see chapter \ref{sec:mylabel2}), it can be documented in the section \textsl{[OTF{\_}MENU]}.\index{on-the-fly menus!in helpfiles}
...
When a submenu has menu specific help, the `\textsl{\lt?\gt}' icon will be shown at the left
in the menu's title bar.

\vs

Helpfiles are read with the \textsf{INFO}\index{OMI commands!INFO} command. For
more information on this command, refer to section~\ref{subsec:cmdinfo}.


\vs

The menu \textsl{OMI{\$}MANAGE}\index{omi\$manage menu}, which comes with the distribution,
...
\indent\texttt{{\$} DELETE /SYMBOL /LOCAL OMI{\$}RECORD5}\\[3mm]
...
configuration file. This key is generated with the \textsf{SET KEY}\index{OMI 
...
Message data files can be created and filled with the \textsl{OMI{\$}MANAGE}\index{messages}\index{omi\$manage menu} menu\footnote{ This menu is not documented, but comes with a basic OMI help file. Type \textsf{INFO} in the menu for more information.}.\\[3mm]
\textbf{Parameters}
...
\textsl{OMI{\$}DECNET{\_}ENABLED}\index{DECnet}\index{OMI{\$}DECNET{\_}ENABLED} & 
\textsl{OMI{\$}{\_}TRUE} if DECnet has been started on the current node, {OMI{\$}{\_}FALSE} otherwise. \\
...
\textsl{OMI{\$}VALIDATE{\_}MODE}\index{OMI mode!validate}\index{validate mode}\index{OMI{\$}VALIDATE{\_}MODE} & 
This symbol is TRUE when OMI is started in validate hmode (see section \ref{subsubsec:mylabel8}), FALSE otherwise. \\
...
\textsl{OMI{\$}{\_}IS{\_}CAPTIVE}\index{OMI{\$}{\_}IS{\_}CAPTIVE}\index{OMI symbols!local symbols!OMI{\$}{\_}IS{\_}CAPTIVE}\index{security!captive accounts}\index{captive accounts} & 
\textsl{TRUE} if the current user has a captive account. \\
...
\textsl{OMI{\$}{\_}IS{\_}RESTRICTED}\index{OMI{\$}{\_}IS{\_}RESTRICTED}\index{OMI symbols!local symbols!OMI{\$}{\_}IS{\_}RESTRICTED}\index{security!restriced accounts}\index{restriced accounts} & 
\textsl{TRUE} if the current user has a restriced account. \\
...
\begin{table}[h!tb]
\begin{minipage}[h!tb]{\textwidth}
\hspace{-15mm}\begin{tabular}{lp{60mm}l} \hline
...
\end{tabular}
\caption{Symbol for font- and color selection}\label{tab:fontsymbols}\index{OMI symbols!fonts and colors}
\end{minipage}
\end{table}
...
\section{CALC}
\index{OMI commands!CALC}\index{calculator}
...
If the result contains a floating point, the result can be rounded. By default this will be 9 digits after the decimal point, this can be changed using the \textsl{SET PRECISION} command (see section \ref{subsubsec:setprecision}).
...
\textsf{SILENT{\_}DCL}\index{OMI commands!SILENT{\_}DCL}.

\vs

In stead of the command \textsf{DCL}, the dollar sign 
(\textsf{{\$}})\index{{\$}}\index{dollar-sign} can also be used.
...
\section{DECRYPT}
\index{OMI commands!DECRYPT}
\index{security!decrypting}
\index{decrypting}
\label{subsec:decrypt}

\indent Format: \textbf{DECRYPT \textit{section-name element-name key-name}}\dag\\*[3mm]
Decrypt the specified element in the menu file. The decrypted value is 
written to the menu file. Therefore, \textsl{WRITE} privilege to the current menu is required.

The key used for decrypting must be the same that was used to encrypt the element (see section \ref{subsec:encrypt}).

\vs 
 
\noindent\textbf{Parameters}\\[3mm]
\textbf{\textit{section-name}}\\The section which holds the element that 
should be decrypted. Square brackets can be omitted.\\[3mm]
\textbf{\textit{element-name}}\\The element name.\\[3mm]
\textbf{\textit{key-name}}\\The name of the key with which the element was encrypted. It has to exist at the keyring of the user's configuration file. If it's not there, it can be imported with the OMI command \textsf{IMPORT KEY}.
...
an own key using the \textsf{SET KEY}\index{OMI commands!SET KEY} command.
...
modules!OMI{\$}DECRYPT} (see section \ref{subsubsec:mylabel50}).
...
\noindent\textbf{Parameters}\\[3mm]
\textbf{\textit{section-name}}\\The section which holds the element that 
should be encrypted. Square brackets can be omitted.\\[3mm]
\textbf{\textit{element-name}}\\The element name.\\[3mm]
\textbf{\textit{key-name}}\\The name of the key with which the string has to be encrypted. It has to exist at the keyring of the user's configuration file. If it's not there, it can be imported with the OMI command \textsf{IMPORT KEY}, or created with the \textsf{SET KEY} command.
...
\textsf{EXIT}\index{OMI commands!EXIT} and \textsf{QUIT}\index{OMI commands!QUIT} are 
synonymous.
...
If the current menu has an info section in the menu specific help\footnote{ This can be seen in the
menu's title bar where the `\textsl{\lt?\gt}' icon will be shown at the left when help is available for this submenu.}
...
As a synonym, a question mark (\textbf{?}) can be entered.
...
\textsf{SHOW NAME}\index{OMI commands!SHOW NAME}\index{name}\index{menu!name}
...
\textsf{QUIT}\index{OMI commands!QUIT} and \textsf{EXIT}\index{OMI commands!EXIT} are 
synonymous.
...
\indent Format: \textbf{SET KEY \textit{name} \textit{value}}\\*[3mm]
This command defines a personal key. This key will be used for the 
\textsf{ENCRYPT} and \textsf{DECRYPT}\index{OMI commands!ENCRYPT}\index{OMI commands!DECRYPT} commands in OMI, and for the 
\textsl{OMI{\$}ENCRYPT}\index{OMI commands in modules!OMI{\$}ENCRYPT} and 
\textsl{OMI{\$}DECRYPT}\index{OMI commands in modules!OMI{\$}DECRYPT} command in OMI modules.
...
section \ref{subsubsec:shared}).

\vs

\noindent\textbf{Parameters}\\[3mm]
\textbf{\textit{name}}\\The name of the key\\[3mm]
\textbf{\textit{value}}\\The value, which must be an integer value between 1 and 1,000,000.
...
\index{OMI commands!SET PRECISION}\index{calculator!precision}
\label{subsubsec:setprecision}

\indent Format: \textbf{SET PRECISION \textit{value} [[NO]INTERMEDIATE\_STEPS]}\\*[3mm]
Set the precision used by the calculator. By default, all floating point
values will be rounded at 9 digits behind the decimal point. That value
can be set with this command.\\
The precision must be between (and including) 1 and 9.

\vs

An optional parameter \textsl{[NO]INTERMEDIATE\_STEPS} can be used to round all
intermediate steps of a calculation.\\
Enabling this will reduce the chance for \textit{out of range} errors and warnings,
but will make the end result less accurate.\\
...
\indent Format: \textbf{SHOW ORDER}\\*[3mm]
...
\subsection{SHOW PRECISION}
\index{OMI commands!SHOW PRECISION}\index{calculator!precision}
\label{subsubsec:showprecision}

\indent Format: \textbf{SHOW PRECISION}\\*[3mm]

Show the current precision  used by the calculator for rounding. This command
also shows if intermediate steps will be rounded.
...
[MENU_MENU]
  ...
  comment = The command INFO can be used in some menus for context specific help
...
\chapter{Example OMI Menu specific help}

This example is included in the distribution as \textsl{OMI{\$}EXAMPLE.OMH}.
...
<ESC>[1mProtected menu<ESC>[0m
    This menu is password protected. The default password as set in the
    distribution is <ESC>[1mtest123456789<ESC>[0m.

<ESC>[1mGet some inputs<ESC>[0m
    This menu and its submenu shows some examples of the way input can be
    asked and formatted.
    It also has <ESC>[4mRESET VARIABLES<ESC>[0m option. An <ESC>[4mON_EXIT<ESC>[0m element checks if this
    reset has been executed.

The submenus <ESC>[1mProtected menu<ESC>[0m and <ESC>[1mCounter menu<ESC>[0m have their own context
specific help.

[PWDTEST]

The default password of this menu is <ESC>[1mtest123456789<ESC>[0m.
The command <ESC>[4mSET PASSWORD<ESC>[0m can be used to change this.

[CNTTEST]

This menu has a counter that is increased every time it is called.

The submenu <ESC>[1mMore counters<ESC>[0m has a different counter and shows an example how
...
$	omi$record4 = ""
$	omi$record5 = " The integer you entered was  : ''inp_int'"
$	omi$record6 = " You called the Counter menu ''counter$another_counter' times"
$!
$!	Make sure the list with months won't be redisplayed, starting with rec. 5
$	if f$type(omi$record7) .nes. "" then -
	   deletee/symbol/local omi$record7
$!
$!	... and display them
$	omi$display_info
$	omi$wait
...
\section*{Version 2.9, Released: xx-xx-202x}

\begin{itemize}
\item Fix: Crash when making a selection from a taglists with sessionlog enabled.
\item Fix: Make resizing the screen work when using Telnet/SSH clients.
\item Fix: Invalid calculator results when the decimal start with zeros.
\item Fix: Number not recognized when the first character of a calculation (step) is a '-' sign.
\item Fix: Known issues with the \textsl{TEXTAREA} input type and \textsl{OMI{\$}DISPLAY{\_}INFO} fixed.
\item Minimize the \textsl{\%OMI-W-PROOUTRA} warnings in the calculator.
\item Implemented rounding options for the calculator and added \textsl{SET PRECISION} and \textsl{SHOW PRECISION} commands.
\item Added an example for context specific help to the \textsl{OMI{\$}EXAMPLE} menu.
\item Added \textsl{OMI{\$}WRAPPER.COM}\index{wrapper}\index{OMI{\$}WRAPPER}, a wrapper for OMI that acts a very simplified try-catch for debugging\footnote{ This wrapper is now always enabled since it can also be useful for troubleshooting (see the note in section \ref{subsubsec:mylabel1}.}.
\item Added the symbol \textsl{OMI{\$}DECNET{\_}ENABLED}.
\item Display an icon in the title bar if menu specific help is available.
\item Added `\textsl{?}' as a synonym for the \textsl{INFO} command.
\item Always refresh the menu after a counter reset.
\item Added security measures for Captive and Restricted accounts with configuration settings for Restricted accounts (see section \ref{subsubsec:restracc}).
\item Implemented the \textsl{DECRYPT} command.
\end{itemize}

\section*{Version 2.8, Released: 04-02-2021}
...
\item Added the \textsf{/[NO]SESSION{\_}LOG} qualifier.
\item Added the \textsf{/REPLAY} option
\item Added \textsf{OMI{\$}DELAY} to the standard toolbox
\end{itemize}
...
\item Search order for modules that are called without explicit path is now reversed: first check in \textsl{OMI{\$}MENU{\_}DIRECTORY}, if not found look for the module in \textsl{OMI{\$}}.
...
\section*{Version 1.0b1, Date: 14-03-1997\footnote{ This version was not publicly released\label{notrel}}}
...
\section*{Version 0.0, Date: 11-03-1997\footref{notrel}}

Initial release, containing submenus, commands with input option, free inputs and selection lists.

\section*{OMB -- Oscar's Menu Builder (1990-1995)\footref{notrel}}
