...
\noindent\textbf{{\copyright} 1997-2021\\
...
\textsf{ENCRYPT}\index{OMI commands!ENCRYPT} command in OMI, and for the 
...
\item Implemented rounding options for the calculator and set \textsl{SET PRECISION} and \textsl{SHOW PRECISION} commands.
\item Added an example for context specific help to the \textsl{OMI{\$}EXAMPLE} menu.
\item Added \textsl{OMI{\$}WRAPPER.COM}, a wrapper for OMI that acts a very simplified try-catch for debugging\footnote{ This wrapper is now always enabled since it can also be useful for troubleshooting.}.
\end{itemize}
